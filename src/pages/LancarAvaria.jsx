...
import { supabase } from '../supabase'; // Importe seu client supabase
// import { Upload } from 'lucide-react'; // Exemplo de ícone

// Componente reutilizável para a linha do orçamento
  ...
  return (
    <div className="grid grid-cols-12 gap-3 mb-2 items-center">
      <input
        ...
        className="col-span-5 border rounded-md px-3 py-2" // Estilo do CentralTratativas
      ...
      <input
        type="number"
        placeholder="Qtd"
        className="col-span-2 border rounded-md px-3 py-2" // Estilo do CentralTratativas
        value={item.qtd}
        min="0"
...
export default function LancarAvaria() {
  ...
  const [formData, setFormData] = useState({
    prefixo: '',
    motoristaId: '', // TODO: Carregar motoristas
    dataAvaria: '',
    tipoOcorrencia: '', // TODO: Carregar tipos
    descricao: '',
  });
  ...
  const handleSubmit = async (e) => {
    ...
    // 1. Fazer upload dos arquivos para o bucket 'avarias'
    const uploadedFileUrls = [];
    for (const file of arquivos) {
      const fileName = `${Date.now()}_${file.name}`;
      const { data: uploadData, error: uploadError } = await supabase.storage
        .from('avarias') // Bucket correto
    ...
    const { data: avariaData, error: avariaError } = await supabase
      ...
      .insert({
        ...formData,
        status: 'Pendente de Aprovação', // Status inicial
        urls_evidencias: uploadedFileUrls, // Array de URLs
    ...
    // 3. Salvar itens do orçamento (Peças e Serviços)
    const orcamentoItens = [
      ...pecas.map(p => ({ ...p, tipo: 'Peca', avaria_id: avariaId })),
      ...servicos.map(s => ({ ...s, tipo: 'Servico', avaria_id: avariaId })),
    ];
    
    // Assumindo que 'id' e 'descricao' não são colunas no Supabase, removemos
    const itensParaSalvar = orcamentoItens.map(({ id, ...rest }) => rest);

    const { error: orcamentoError } = await supabase
      .from('cobrancas_avarias') // Tabela de itens do orçamento
      .insert(itensParaSalvar);

    if (orcamentoError) {
      console.error('Erro ao salvar orçamento:', orcamentoError);
      // Aqui talvez seja necessário deletar a avaria criada (rollback manual)
      alert('Falha ao salvar itens do orçamento: ' + orcamentoError.message);
    } else {
      alert('Avaria lançada para aprovação com sucesso!');
      // TODO: Limpar formulário ou navegar para outra página
      // navigate('/central-tratativas');
    }
  ...
  return (
    // Container principal (baseado no CentralTratativas)
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white shadow rounded-lg p-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            {/* TODO: Substituir por Selects buscando do Supabase */}
            <div className="flex flex-col">
              <label htmlFor="prefixo" className="mb-1 text-sm font-medium text-gray-600">Prefixo</label>
              <input type="text" name="prefixo" id="prefixo" className="border rounded-md px-3 py-2" onChange={handleFormChange} required />
            </div>

            <div className="flex flex-col">
              <label htmlFor="motoristaId" className="mb-1 text-sm font-medium text-gray-600">Motorista</label>
              <input type="text" name="motoristaId" id="motoristaId" className="border rounded-md px-3 py-2" onChange={handleFormChange} required />
            </div>
        ...
        <div className="flex justify-end gap-4 pt-4">
          <button
            type="button"
            onClick={() => { /* TODO: Adicionar reset/limpar */ }}
