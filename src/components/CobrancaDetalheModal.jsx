...
export default function CobrancasAvarias() {
  ...
  const carregarCobrancas = async () => {
    ...
    // AJUSTE DE COLUNA: se no banco for "data_avaria", troque "dataAvaria" por "data_avaria"
    if (dataInicio) {
  ...
  const carregarResumo = async () => {
    ...
    if (dataInicio) {
      query = query.gte("dataAvaria", dataInicio); // ajustar nome se necessário
    }
    if (dataFim) {
      const fimISO = `${dataFim}T23:59:59`;
      query = query.lte("dataAvaria", fimISO); // ajustar nome se necessário
    }
